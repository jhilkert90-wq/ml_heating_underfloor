...
class TestSlabThermalStateFeatures:
    ...
    def test_d_inlet_temp_60min_computed_from_rlt_diff(self):
        ...
        df = _make_df(800, at_val=8.0)
        # Set a linearly increasing RLT so diff(6) = 1.0 per step
        ...
        with patch(
            ...
            mock_cfg.HEAT_LOSS_COEFFICIENT = 0.124
            mock_cfg.THERMAL_TIME_CONSTANT = 4.39
            mock_cfg.HEATING_ML_CORRECTION_MODEL_PATH = "/tmp/test_slab.joblib"
            mock_cfg.HEATING_ML_CORRECTION_METADATA_PATH = "/tmp/test_slab_meta.json"
            mock_cfg.INDOOR_TEMP_ENTITY_ID = "sensor.indoor"
            mock_cfg.OUTDOOR_TEMP_ENTITY_ID = "sensor.outdoor"
            mock_cfg.OUTLET_TEMP_ENTITY_ID = "sensor.outlet"
            mock_cfg.INLET_TEMP_ENTITY_ID = "sensor.inlet"
        ...
        if "X" in captured_X and hasattr(captured_X["X"], "columns"):
            ...
            # With linearly increasing RLT (diff(6) ≈ 1.0), is_equilibrium should be 0.0
            assert captured_X["X"]["is_equilibrium"].max() == 0.0
    ...
    def test_is_equilibrium_is_1_when_rlt_stable(self):
        ...
        with patch(
            ...
            mock_cfg.HEAT_LOSS_COEFFICIENT = 0.124
            mock_cfg.THERMAL_TIME_CONSTANT = 4.39
            mock_cfg.HEATING_ML_CORRECTION_MODEL_PATH = "/tmp/test_slab2.joblib"
            mock_cfg.HEATING_ML_CORRECTION_METADATA_PATH = "/tmp/test_slab2_meta.json"
            mock_cfg.INDOOR_TEMP_ENTITY_ID = "sensor.indoor"
            mock_cfg.OUTDOOR_TEMP_ENTITY_ID = "sensor.outdoor"
            mock_cfg.OUTLET_TEMP_ENTITY_ID = "sensor.outlet"
            mock_cfg.INLET_TEMP_ENTITY_ID = "sensor.inlet"
        ...
        if "X" in captured_X and hasattr(captured_X["X"], "columns"):
            ...
            assert captured_X["X"]["is_equilibrium"].min() == 1.0
