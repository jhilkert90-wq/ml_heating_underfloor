...
### Fixed
- **Review-thread follow-ups**: `check_and_resolve_climate_mode()` now derives `heating_active` from the raw climate mode to avoid redundant Home Assistant reads and only invokes the HA state-check side effect in the explicit `off` case; the warm-restart sentinel is also cleared when a fresh wrapper starts in the same mode as a stale sentinel left from a prior restart.
...
### Added
- **`shading_proxy` feature**: Continuous solar shading proxy (`max(0, T_indoor−23) × PV_W`, units K×W) added to heating correction ML pipeline (calibration + inference). Captures solar overheat protection (Übergangszeit) more accurately than hard binary flags.
...
- **`thermal_power_w` feature**: Replaced `thermal_power_kw` feature with `thermal_power_w` (= thermal_power_kw × 1000) so all thermal power features are consistently in Watts.
...
- `thermal_power_rolling_1h` now computed from the W column in calibration and returns kW×1000 at inference — consistent units (W) with `thermal_power_w`.
- `shading_proxy` formula updated: `PV_Generate` is in W, so `/1000` division removed; units now K×W instead of K×kW.
