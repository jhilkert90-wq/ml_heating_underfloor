...
- ✅ `config_adapter.py`: added mappings for `ELECTRICITY_PRICE_ENTITY_ID`, `TREND_DECAY_TAU_HOURS`, `PV_ROOM_DECAY_MULTIPLIER`, `DECAY_CANCEL_MARGIN`, `PV_TRAJ_SEASONAL_SCALING_ENABLED`, `PV_TRAJ_LATITUDE`, `PV_TRAJ_SEASONAL_MIN_FACTOR`; removed deprecated `safety_max_temp`/`safety_min_temp` dead-code validation
- ✅ `ml_heating_underfloor/config.yaml`: added `electricity_price_entity`, `trend_decay_tau_hours`, `pv_room_decay_multiplier`, `decay_cancel_margin`, `pv_traj_seasonal_scaling_enabled`, `pv_traj_latitude`, `pv_traj_seasonal_min_factor` to both `options:` and `schema:`
