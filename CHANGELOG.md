...
- **PV routing at sunset**: `_is_pv_active()` now uses `max(pv_power_current, pv_power_smoothed)` against `PV_LEARNING_THRESHOLD` (default 50W). This captures solar thermal lag where smoothed PV stays high after instantaneous PV drops.
